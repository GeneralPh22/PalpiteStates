...
import { Triangle, Clock, RefreshCw } from "lucide-react";
...
function CornerBar({ value, max = 100 }: { value: number; max?: number }) {
  const pct = Math.min(100, (value / max) * 100);
  const color = value >= 70 ? "bg-sky-400" : value >= 55 ? "bg-sky-500" : "bg-sky-700";
  return (
    <div className="flex items-center gap-2">
      <div className="flex-1 h-1.5 bg-white/[0.06] rounded-full overflow-hidden">
        <div className={`h-full rounded-full ${color} transition-all`} style={{ width: `${pct}%` }} />
      </div>
      <span className={`text-sm font-bold tabular-nums w-10 text-right ${value >= 70 ? "text-sky-300" : "text-white/70"}`}>
        {value}%
      </span>
    </div>
  );
...
function MatchCard({ match, idx }: { match: CornerMatch; idx: number }) {
  ...
  return (
    <motion.div
      ...
      className="bg-white/[0.03] rounded-xl border border-white/[0.07] hover:border-sky-500/30 transition-colors p-3"
    >
      ...
      <Link href={`/fixture/${match.fixtureId}`}>
        <div className="flex items-center gap-1.5 mb-2.5 cursor-pointer group">
          ...
          <span className="text-sm font-semibold text-white group-hover:text-sky-400 transition-colors truncate">{match.homeTeam}</span>
          ...
          <span className="text-sm font-semibold text-white group-hover:text-sky-400 transition-colors truncate">{match.awayTeam}</span>
        </div>
      </Link>

      <div className="flex items-center gap-2 mb-2.5 text-[11px] text-white/40">
        <span>Média escanteios:</span>
        <span className="text-white/70 font-medium">{match.homeAvg} + {match.awayAvg}</span>
        <span className="text-sky-400 font-semibold ml-auto">= {match.totalAvg} total</span>
      </div>

      <div className="space-y-1.5">
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-white/40 w-24 flex-shrink-0">Over 8.5</span>
          <CornerBar value={match.over85Pct} />
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[10px] text-white/40 w-24 flex-shrink-0">Over 9.5</span>
          <CornerBar value={match.over95Pct} />
        </div>
      </div>
    </motion.div>
  );
...
export default function CornerScannerSection() {
  const { data, isLoading, isError, refetch, isFetching } = useQuery<ScannerData>({
    queryKey: ["scanner-corners"],
    queryFn: async () => {
      const r = await fetch(`${BASE}/api/scanner/corners`);
      ...
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <div className="w-7 h-7 rounded-lg bg-sky-500/20 flex items-center justify-center">
            <Triangle className="w-3.5 h-3.5 text-sky-400 fill-sky-400/30" />
          </div>
          <h2 className="text-base font-bold text-white">Alta Probabilidade de Escanteios</h2>
          <span className="text-[10px] bg-sky-500/15 text-sky-400 border border-sky-500/20 rounded-full px-2 py-0.5 font-medium">
