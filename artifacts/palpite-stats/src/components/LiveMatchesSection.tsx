...
import { Radio, ChevronDown, ChevronUp, Flame } from "lucide-react";
...
// ── Goal Pressure Index ────────────────────────────────────────────────────────
// Weights: SoT 40% | Shots 20% | Corners 15% | Possession dominance 15% | Dangerous attacks 10%
...
function calcTeamGPI(stats: TeamStats): number {
  ...
  const shotsScore    = Math.min(20, (stats.shots / 15) * 20);
  const cornersScore  = Math.min(15, (stats.corners / 8) * 15);
  const possScore     = Math.max(0, ((possNum - 50) / 50) * 15);
  const attackScore   = Math.min(10, ((stats.dangerousAttacks ?? 0) / 30) * 10);
  return Math.round(Math.min(100, soTScore + shotsScore + cornersScore + possScore + attackScore));
}
...
function calcGoalProbs(elapsed: number, matchGPI: number, totalGoals: number) {
  ...
  const baseRate       = 0.030;                           // goals/min average
  const intensity      = 1 + matchGPI / 100;             // 1.0× – 2.0×
  const lambdaPerMin   = baseRate * intensity;
...
function MomentumBar({ label, gpi, color }: { label: string; gpi: number; color: string }) {
  return (
    <div className="space-y-0.5">
      <div className="flex items-center justify-between text-[9px]">
        <span className="text-white/40 truncate max-w-[100px]">{label}</span>
        <span className={cn("font-black tabular-nums ml-2", color)}>{gpi}</span>
      </div>
...
function ProbCard({ label, value }: { label: string; value: number }) {
  ...
  return (
    <div className={cn(
      ...
      <div className={cn("text-xs font-black tabular-nums", hi ? "text-amber-400" : "text-white/50")}>{value}%</div>
    </div>
  );
}
...
function LiveMatchCard({ match, idx }: { match: EnrichedMatch; idx: number }) {
  ...
  return (
    ...
    >
      {/* GPI intensity bar across top */}
      {hasStats && match.matchGPI > 0 && (
      ...
      <div
        className={cn(
          ...
          hasStats && "cursor-pointer hover:bg-white/[0.03] active:bg-white/[0.05]"
        )}
      ...
      {expanded && hasStats && match.stats && (
        <div className="border-t border-white/[0.05] px-3 pb-3 pt-2 space-y-3">
          ...
          {/* ── Attacking Momentum bars ── */}
          <div className="space-y-1.5">
            <span className="text-[8.5px] text-white/20 uppercase tracking-widest font-semibold">
              Pressão Ofensiva
            </span>
            <MomentumBar label={match.homeTeam} gpi={match.homeGPI} color={gpiColor(match.homeGPI)} />
            <MomentumBar label={match.awayTeam} gpi={match.awayGPI} color={match.awayGPI > 75 ? "text-red-400" : match.awayGPI > 50 ? "text-amber-400" : "text-blue-400/70"} />
          </div>
